...
LastEditTime: 2024-04-20 15:21:56
...
class DecentralizedModels():
    def __init__(self, base_model, nodesize, device) -> None:
    ...
    def __getitem__(self, node):
        return self.decentralized_mdoels[node]
    ...
    def to_vectors(self, node = None):
        if node is not None:
            result = torch.zeros([self.modelsize], dtype=FEATURE_TYPE)
            result.copy_(
                    torch.cat([p.flatten() for p in self.decentralized_mdoels[node].parameters()])
                )
            return result
    ...
    def update_model(self, param_vector, node):
        """ update the client node use parame_vector
        param_vector[torch]
        """
        beg = 0
        end = 0
        for p in self.decentralized_mdoels[node].parameters():
            end = beg + p.nelement()
