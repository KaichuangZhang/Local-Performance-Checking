...
from optimization_algorithms.CentralizedModels import CentralizedModels
...
class CentralizedSGD():
    ...
    def run(self):
        # models
        self.d_models = CentralizedModels(self.model, self.graph.node_size, device=self.device)
        ...
        for i in range(total_iterations + 1):
            ...
            for node in self.graph.honest_nodes:
                ...
                for k in range(self.local_rounds):
            ...
            # Byzantine attack
            byzantine_neighbors_size = self.graph.byzantine_sizes[self.graph.center_node]
            if self.attack != None and byzantine_neighbors_size != 0:
                self.attack.run(d_models_vector, self.graph.center_node)
            # aggregation
            #gaussian_noise = torch.randn_like(param_bf_comm) * lr * self.noise_level
            if i == 0:
                log.info(f'center node[{self.graph.center_node}] - neighbors{self.graph.neighbors[self.graph.center_node]} byzantine neighbors{self.graph.byzantine_neighbors[self.graph.center_node]}')
            aggregation_res = self.aggregation.run(d_models_vector, self.graph.center_node)
            self.d_models.update_all_models(aggregation_res)
    ...
    def test_by_nodes(self):
        ...
        for node in self.graph.honest_nodes:
            ...
            if node not in test_info:
                test_info[node] = {
                    'node': node,
                    'accuracy': accuracy / len(samples_y) * 1.,
                    'loss': loss
                }
        ...
        test_info['average loss'] /= self.graph.honest_size
        test_info['test accuracy variance'] = torch.var(torch.tensor(test_info['test accuracy']), unbiased=False)
        return test_info