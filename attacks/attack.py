...
def get_model_control(messages, honest_nodes, byzantine_nodes, target_message):
    ...
    # [target updates * len(honest_nodes) - s ] / len(byzantine_nodes)
...
class D_isolation(decentralizedAttack):
    ...
    def run(self, local_models, node, torch_rng=None):
        byzantine_neigbors = self.graph.byzantine_neighbors[node] # get the byzantine neigbors
        melicious_message = get_dec_model_control(self.graph, local_models, node,  
                                                  local_models[node])
        for n in byzantine_neigbors:
            local_models[n].copy_(melicious_message)
...
class D_IPM(decentralizedAttack):
    def __init__(self, graph, epsilon):
        if epsilon is None:
            name = 'alie'
        else:
            name = f'alie_epsilon={epsilon}'
        super().__init__(name=name, graph=graph)
